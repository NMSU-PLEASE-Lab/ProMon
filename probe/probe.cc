...
int sendInstRecord(const char *args, const char *tag) {
	...
	/*
	 * The tag that is passed to this function from the monitored application is
	 * in this format: SamplingRate:Priority:Category:name 
	 * The sent event has only Name or in the case of DATA_ACESS some extra data in format
	...
	sprintf(tmp, "%s", tag);
	samplingRate = strtok(tmp, ":");
	priority = strtok(NULL, ":");
	category = strtok(NULL, ":");

	pch = strtok(NULL, ":");
	while (pch != NULL) {
		strippedTag += ":" + string(pch);
		pch = strtok(NULL, ":");
	}
	
	/* Then Send the Name tag to the functions below */
	...
	if ((strcmp(comm_type, "TCP") == 0) || (strcmp(comm_type, "EPOLL") == 0)) {
		sendInstRecordTCP(strippedTag.c_str());
	}
	else if (strcmp(comm_type, "MSGQUEUE") == 0) {
		sendInstRecordMSGQUEUE(strippedTag.c_str());
	} else if (strcmp(comm_type, "POSIX") == 0) {
		sendInstRecordPOSIX(strippedTag.c_str());
	} else {
		sendInstRecordUDP(strippedTag.c_str());
	}
