...
void Parser::monElement(TiXmlElement* pElem, instRecord* record1,
	...
	for (; pElem; pElem = pElem->NextSiblingElement()) {
		...
		if (strcmp(pKey, "SamplingRate") == 0) {
			...
			if (record1->type == "DUAL") {
				...
				record1->name = SINGULAR_BEGIN + string(pText);
	...
	if (hasVariable && (hasBeginPoint || hasEndPoint)) {
		ProMon_logger(PROMON_ERROR, "ProMon Parser: Monitoring elements that reads variables only can have Point to define their position!!");
		return;
	}
 ...
 * The name of the event will be discarded and will be named DATA_ACCESS
 * The format of the name, at this point, will be: DATA_ACCESS:Type:Name
 */
void Parser::variable(TiXmlElement* pElem, instRecord* record) {
	...
	for (; pElem; pElem = pElem->NextSiblingElement()) {
		...
		if (strcmp(pKey, "Type") == 0) {
			...
			record->name = record->name + ":" + string(DATA_ACCESS) + ":" +
				record->variableType + ":" + record->variableName;
		}
	}
	...
	for (; pElem; pElem = pElem->NextSiblingElement()) {
		...
		if (strcmp(pKey, "Function") == 0) {
			...
			if (strcmp(pText, "BEGIN") == 0 && strcmp(pText, "END") == 0) {
				ProMon_logger(PROMON_ERROR, "ProMon Parser: In the Point section, Function or Position is not defined!!");
				return;
			}
