...
void Injector::startInstrumentation(BPatch_addressSpace *app, string strArgv, vector<instRecord> arrangedrecs)
{
         ...
         it != arrangedrecs.end(); ++it)
   {
       ...
       */
      BPatch_variableExpr *variable = NULL; 
      BPatch_constExpr *secondParam = NULL;
      if( (string(it->name)).find(DATA_ACCESS) != string::npos)
      { 
          variable = appImage->findVariable(it->variableName.c_str());
          secondParam = new BPatch_constExpr(variable->getBaseAddr());
          args.push_back(secondParam); //it takes the address only
      ...
      BPatchSnippetHandle *sh; //position
      if (strcmp(it->position.c_str(), "BEGIN") == 0)
      {
         /*
          * If no basic block is defined to be instrumented then
          * instrument the entry points of the function.
          */
         if(it->basicBlockNo == -1 && it->loopNo == -1)
         {
            points = functions[i]->findPoint(BPatch_entry);
            sh = app->insertSnippet(*probeFunction, *points, BPatch_callBefore);
            sh = sh; //avoid the warning  [-Wunused-but-set-variable]

            ProMon_logger(PROMON_DEBUG, "ProMon Injector: insert probe at %s %s position %p size %d ", 
                         it->position.c_str(), it->name.c_str(), sh, points->size());
         ...
         }else if(it->basicBlockNo != -1)
         {
            ...
            ProMon_logger(PROMON_DEBUG, "ProMon Injector: insert probe at basic block %d %s position: %p %s 1",
         ...
         }else /*handling loops*/
         {
            ...
            vector<BPatch_basicBlockLoop *>::iterator loop_iter;
            loop_iter = loops.begin();
            for(int i=0; i < it->loopNo; i++)
               loop_iter++;
            BPatch_basicBlockLoop *loop = *loop_iter;
            /*
             * get the instrumentation point from control flow graph
             */
            points = fg->findLoopInstPoints(BPatch_locLoopEntry, loop);
            sh = app->insertSnippet(*probeFunction, *points);
         }
      } else if (strcmp(it->position.c_str(), "END") == 0)
      {
         /*
          * If no basic block is defined to be instrumented then
          * instrument the exit points of the function.
          */
         if(it->basicBlockNo == -1 && it->loopNo == -1)
         {
            points = functions[i]->findPoint(BPatch_exit);
            sh = app->insertSnippet(*probeFunction, *points, BPatch_callAfter);

            ProMon_logger(PROMON_DEBUG, "ProMon Injector: insert probe at %s %s position: %p %d",
                          it->position.c_str(), it->name.c_str(), sh, points->size());
         ...
         }else if(it->basicBlockNo != -1)
         {
            ...
            ProMon_logger(PROMON_DEBUG, "ProMon Injector: insert probe at basic block %d %s %s position: %p 1 ",
                          it->basicBlockNo, it->position.c_str(), it->name.c_str(), sh);
         ...
         }else /*handling loops*/
         {
             ...
             * get the instrumentation point from control flow graph
             */
            points = fg->findLoopInstPoints(BPatch_locLoopExit, loop);
            sh = app->insertSnippet(*probeFunction, *points);
         }
      }
      i++;
   ...
   if ((argc - 1) < 2)
   {
      ProMon_logger(PROMON_ERROR,
                    "Dynamic instrumentation usage: %s DynamicInst <monspec-xml> <app-binary> [app-argument1 app-argument2 app-argument3 ...]\n \
                      ...
                      Attach instrumentation usage: %s AttachInst  <monspec-xml> <app-binary> <pid> [app-argument1 app-argument2 app-argument3 ...]", argv[0], argv[0], argv[0]);
      return 1;
   }else if( (strcmp("DynamicInst", argv[1]) != 0) && (strcmp("BinaryInst", argv[1]) != 0) && (strcmp("AttachInst", argv[1]) != 0) ) 
   {
      ProMon_logger(PROMON_ERROR,
                    "Dynamic instrumentation usage: %s DynamicInst <monspec-xml> <app-binary> [app-argument1 app-argument2 app-argument3 ...]\n \
                      Static instrumentation usage: %s BinaryInst  <monspec-xml> <app-binary> [app-argument1 app-argument2 app-argument3 ...]\n \
